...
> **Stripe payments (2026-07-23 late) — WORKING except final webhook test:** platform account "Roundmate" (acct_1TwJ6xGIO7bsTn8b, ltd entity, Connect enabled, live). Connected account for Ringo's business acct_1TwOD87dhhAJCvwB (Payments+Payouts active; ~£4 of real £1 test payments in balance). Invoice emails carry a Pay-now payment link (created on the connected account at send time; auto-created via /api/send-email). **Saga:** platform-level connect webhooks would NOT deliver (dashboard destination was your-account scope; even an API-created endpoint showing "Events from: Connected accounts" in the dashboard got zero deliveries despite events firing on the connected account — Stripe-side routing mystery). **Plan B implemented (untested at session end): per-connected-account webhook endpoints** — created ON the connected account, URL /api/stripe/webhook?business=<id>, per-business signing secret in businesses.stripe_webhook_secret (migration applied); handler verifies per-business secret (env STRIPE_WEBHOOK_SECRET remains as legacy fallback); connect route auto-creates the endpoint for future tradesmen. **NEXT SESSION: (1) RESOLVED LATE: the platform connect endpoint we_1TwUro DID deliver (200 OK, {"received":true}) once the whsec was redeployed — loop confirmed working; the "unpaid" £1 job was simply never paid via its own link (all test payments used the first invoice's link). (2) Push the (better) per-account-webhook plan-B commit anyway; optionally visit /api/stripe/setup-webhook to switch Ringo's account to it. (3) BUILD: webhook-triggered branded receipt email — customer who pays by card currently gets NO confirmation from us; extract email building from /api/send-email into a shared lib so the webhook can send the receipt (with photos). Interim: Stripe's own receipts can be enabled on the connected account (Settings→Emails→Successful payments). (4) Cleanup: DELETE /api/stripe/setup-webhook + /api/stripe/debug routes, roll the whsec that passed through chat, refund/tidy £1 test payments, delete orphan test connected accounts.** Env on Vercel: STRIPE_SECRET_KEY (live), STRIPE_WEBHOOK_SECRET, SUPABASE_SERVICE_ROLE_KEY all set.
...
> **Payment reminders (2026-07-24, push pending):** Vercel Cron (vercel.json, daily 08:00 UTC) → GET /api/cron/payment-reminders (CRON_SECRET bearer auth — **Ringo must add CRON_SECRET env var in Vercel**; supports ?dry=1). Chases completed+unpaid jobs at day 3/7/14 after completion (gentle→nudge→firm, pay button + invoice ref, "ignore if already paid" line), tracked via jobs.reminder_count/last_reminder_at (migration applied), max 3 ever, 50/run cap, skips @example.com test customers. sendJobEmail now takes type "reminder" + {reminderNumber}. Consumer-directory brief accepted → RoundMate_Integration_Brief.md + PLAN-premium-features.md Feature 3 (one booking_requests table for booking page + directory, no anon writes, GDPR/PECR flags).

> **Accounting module (2026-07-24):** simple accountancy layer live in code (push pending). Migration applied: `jobs.paid_at/paid_method/invoice_number`, `businesses.next_invoice_number`, atomic `take_invoice_number(b_id)` SQL function (SECURITY INVOKER, returns 'INV-0001'-style). All three paid paths capture date+method: Complete flow (cash/bank), Money "Mark paid" (assumes bank; also now fires receipt email), Stripe webhook (card). Invoices get a sequential per-business number at send time (subject + "Invoice ref" line; receipts show the ref too). Money page has an **Accounts** section (`components/AccountsExport.js`): date-range CSV export (defaults to current UK tax year from 6 April) with work date, invoice no, customer, postcode, description, amount, status, paid date, method — importable into FreeAgent/Xero/QuickBooks; older jobs fall back to payment_status as the method. VAT deliberately excluded (Ringo not registered); future accounting connectors (PLAN-premium-features.md) just map these same fields over OAuth.


> **UI pass + accountant reports (2026-08-23/24) — branch `ui-accountant`, PR open, NOT yet merged to `main`:** **UI:** fixed bottom tab bar (Today/Jobs/Money/Customers/More — `components/BottomNav.js`, mounted in the root layout, hidden on `/`, `/login`, `/onboarding`, respects the iPhone home-bar inset); `loading.js` skeletons for root/dashboard/jobs/money/customers (`components/Skeleton.js`) plus `app/error.js` and `app/not-found.js` — a failure used to be a blank white screen; `lib/money.js` `gbp()` via `Intl.NumberFormat` so £62.5 renders £62.50 everywhere including invoice/receipt emails; overdue day-count badges on `/money` (grey 3 / amber 7 / red 14, matching the reminder cron) with the list sorted oldest-debt-first and reminder counts shown; Complete-job button now says what it will do ("Complete & email invoice · £25") with the payment picker as a 2×2 `aria-pressed` group; Call + Navigate (`components/ContactActions.js`, `tel:` + maps deep link) on job and customer screens; actionable empty states; Sign out moved off the dashboard into Settings. **CSS:** `button` and `.btn` share one rule so a `<Link className="btn">` looks like a button — all 17 invalid `<Link><button></Link>` nests removed; added `:focus-visible`, `100dvh`, `.badge`, `.empty`, `.skel`, `.actions-row`. **Assets:** dashboard tile icons re-cropped (2–9px of the design mock's own tile border was baked into the PNGs — that faint line above each icon); locksmith and pest control finally have real images (they were one duplicated window-cleaner placeholder).

> **Accountant reports — Stage 0.5 (2026-08-23), between the CSV export and the planned FreeAgent OAuth:** migration `sql/2026-08-20-accountant.sql` (**RUN 2026-08-23**) adds `businesses.accountant_name / accountant_email / accountant_frequency / accountant_last_sent_at`. Settings gets a **My accountant** card (`components/AccountantSettings.js`) — name, email, and only-when-I-tap / monthly / quarterly. `/money` → Accounts gains **"Send to …"**, which emails the chosen date range as a CSV attachment with a totals summary (jobs, invoiced, received, outstanding). `/api/cron/accountant-report` runs 09:00 on the 1st: monthly = the month just gone; quarterly fires only in Jan/Apr/Jul/Oct and covers the three months just gone; an empty period is skipped quietly, not reported as a failure. Shared builder/sender in `lib/accountsReport.js` (also exports `periodFor`), tradesman-facing route `app/api/accounts-report/route.js`. **Verified end to end — real CSV email delivered 2026-08-23.** Note the CSV columns are deliberately duplicated between `components/AccountsExport.js` (client, for the download) and `lib/accountsReport.js` (server, for the email) — change one, change both. VAT still excluded.

> **Email was silently broken — found 2026-08-23:** the `RESEND_API_KEY` in Vercel belonged to a Resend account where `roundmate.co.uk` is **not** verified, so every send — invoice, receipt, confirmation, reminder — had been returning `403 validation_error` for an unknown period. It went unnoticed because `CompleteJobForm.fireEmail()` swallowed failures without checking `res.ok`: a job would complete, the app would say done, and the invoice never left. **Both fixed:** correct key pasted into Vercel, and completion / Money "Mark paid" now report a failed send (job still saves first — a bad signal must never cost the completion; a customer with no email is skipped rather than reported as a failure). Retry lives on the job page via the existing `SendEmailButtons`. **Vercel env vars are baked into a deployment at build time** — changing one needs a redeploy, and must be set for Production, Preview AND Development or previews keep failing. **"Sensitive" Vercel variables cannot be read back**, only overwritten.

> **DNS / deliverability (2026-08-24):** DMARC added — TXT `_dmarc` = `v=DMARC1; p=none` (monitor only, safe). **SPF and DKIM were already correct and must NOT be "fixed":** Resend sends via the `send.` subdomain (TXT `send` = `v=spf1 include:amazonses.com ~all`, MX `send` = `feedback-smtp.eu-west-1.amazonses.com`), DKIM signs as the root domain via `resend._domainkey`, and the root TXT SPF (`include:_spf.mail.hostinger.com`) is for the Hostinger mailboxes. Two separate mail paths, each with its own SPF — merging them breaks Resend. A domain may only ever have one SPF record per name. First accountant email landed in the Sky **junk** folder: that is young-domain reputation, not authentication. Improves with steady volume and recipients marking not-junk; tighten to `p=quarantine` later once confident.

> **Vercel gotcha (2026-08-24):** cron jobs are registered from the **production** deployment's `vercel.json` only, so `/api/cron/accountant-report` will not appear in the Cron Jobs tab until `ui-accountant` merges to `main` — not a bug. `CRON_SECRET` **is now set** (superseding the to-do in the 2026-07-24 entry above) and the payment-reminders cron is confirmed running at 08:00 — its Observability trace shows an outbound call to `tzlscfbfpfvginwdphru.supabase.co`, which is only reachable past the secret check. **As of 2026-08-24 there are 0 completed-unpaid jobs and `reminder_count` is 0 across the board — no reminder has ever actually sent.** The chain is unit-tested and the route runs, but it has never been exercised against a real overdue job.

> **Where things actually are (2026-08-24), superseding the older Deployment section below:** local repo `~/Projects/Pug` (NOT `~/Documents/...`); GitHub `ring120768/Roundmate`; Vercel project **`roundmate`** (team `ring120768-2588s-projects`) → https://roundmate.co.uk. Beware: a separate Vercel project **`roundmate-portal`** exists for the consumer directory — near-identical name, different app, easy to debug the wrong one for half an hour. A test suite for the pure logic (`tests/unit.mjs` + an `npm test` script, 51 tests covering `gbp()`, overdue thresholds, the cron's date maths and the accountant report against a stubbed Resend) was written on 2026-08-23 but **was never applied to the repo** — it exists only as `add-tests.patch` from that session.
## What this is
