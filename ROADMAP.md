...
**Route optimisation (Smart Rounds phase 2) — RESEARCHED 2026-07-23, two-stage plan:**
*Stage 1 (FREE, base tier):* geocode customers once at save/import via postcodes.io (free, no key, bulk 100/request, postcode-centroid accuracy — fine for ordering residential rounds); store lat/lng on customers. "Order my day" = nearest-neighbour + 2-opt heuristic on haversine in a route handler (≤25 stops, milliseconds, ~5% off optimal — no external API). Handoff: "Navigate to next stop" deep link (works for Google/Apple/Waze — they only support single-destination links) + optional "Open route in Google Maps" for next ≤10 stops via waypoints URL (test real waypoint limit on a phone before writing UI copy). Total cost £0.
*Stage 2 (premium polish):* swap heuristic internals for Google Routes API computeRoutes with optimizeWaypointOrder (Pro SKU $10/1k, **5,000 free calls/month** post-March-2025 — free at our scale for a long time; max 25 intermediate waypoints; keep DIY heuristic as fallback). Server-side key, daily quota cap in Cloud Console. Geocoding API ($5/1k, 10k free/mo) only for ambiguous addresses. Avoid: Mapbox Optimization (12-stop cap), OpenRouteService (commercial-use grey area), OSRM demo (non-commercial).
