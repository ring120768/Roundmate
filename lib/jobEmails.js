...
export async function sendJobEmail(supabase, jobId, type, { replyTo = null } = {}) {
  ...
  const { data: job } = await supabase
    .from("jobs")
    .select(
      "id, business_id, service_type, price, appointment_date, start_time, photo_paths, payment_link_id, payment_link_url, customers(id, first_name, last_name, email), businesses(name, stripe_account_id)"
    )
    .eq("id", jobId)
    .single();
